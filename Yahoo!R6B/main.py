...
from TSDR_P import TSDR_P
from TSDR_T import TSDR_T
        ...
        with fileinput.input(files) as f:
            for line in f:
                ...
                if len(y_r6) == 8e4:                    
                    ...
                    if config.model == 'TSDR_T':
                        model = TSDR_T(num_users=136, num_items=653, embedding_k=config.embedding_dimention, nu=config.nu)
                    elif config.model == 'TSDR_P':
                        model = TSDR_P(num_users=136, num_items=653, embedding_k=config.embedding_dimention, nu=config.nu)
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-mo', '--model', type=str, default= 'TSDR_T', help='Model name.')
